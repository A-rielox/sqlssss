...
In this example, the EMPLOYEES2 table has been selected as the parent table and has a primary key constraint placed on the EMP_ID column. 

The COMPENSATION table is the child table and its primary key column also has a foreign key constraint which references the EMP_ID column in the EMPLOYEES2. This ensures that EMP_ID values entered into the COMPENSATION table are valid in that they first exist in EMPLOYEES2 table. 

The EMP_ID column also has a primary constraint on it. This ensures that the relationship between the two tables is always one-to-one. Each parent record can only have one child record.
*/
...
Then we would need to ensure that we add the new employee's salary and health insurance plan into the COMPENSATION table:
*/
